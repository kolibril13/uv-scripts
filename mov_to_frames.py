...
def resolve_input_video(downloads: Path, filename: str | None) -> Path:
    if filename:
        ...
        if video_path.suffix.lower() != ".mov":
            raise ValueError(f"Expected a .mov file, got: {video_path.name}")
        return video_path

    mov_files = [p for p in downloads.iterdir() if p.is_file() and p.suffix.lower() == ".mov"]
    mov_files.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    if not mov_files:
        raise FileNotFoundError("No .mov files found in Downloads.")
    return mov_files[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract frames from a MOV in Downloads into a subfolder."
    )
    parser.add_argument(
        "filename",
        nargs="?",
        help="MOV filename in Downloads (example: clip.MOV). Defaults to newest .mov/.MOV.",
    )
