...
# consecutive runs don't overwrite each other (and don't clash with the 24/60fps ones)
output_video = downloads_path / f"{latest_folder.name}_30fps.mp4"
...
try:
    ...
    subprocess.run(
        [
            ...
            # libx264 + yuv420p requires even dimensions; hold last frame 0.5s
            "-vf", "scale=ceil(iw/2)*2:ceil(ih/2)*2,tpad=stop_mode=clone:stop_duration=0.5",
            "-vcodec", "libx264",
            "-pix_fmt", "yuv420p",
            str(output_video),
        ],
